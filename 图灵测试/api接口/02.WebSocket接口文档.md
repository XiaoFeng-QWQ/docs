...
```json
{
  ...
  "msg_type": "markdown",
  "content": "{\"v\":1,\"blocks\":[{\"t\":\"text\",\"text\":\"大家好！\"}]}",
...
| `content` | string | 结构化消息 JSON 字符串：`{"v":1,"blocks":[...]}`，节点结构见《结构化消息解析 SDK》 |

> 文本消息的 `content` 已从纯文本改为结构化 blocks JSON，客户端需按 `msg_type === 'markdown'` 解析 blocks 再渲染。
